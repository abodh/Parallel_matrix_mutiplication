...
int main(int argc, char* argv[])
{
	...
	val_check = 0; //index to check the multiplication value

	while (used_matrix_size <= MAXSIZE)
	{
		//dynamically allocate memory for the resultant multiplication matrix
		...
		for (i = 0; i < used_matrix_size; i++)
		{
			icalc = i * used_matrix_size; // decreases the redundancy of calculating the index inside two other loops
			for (j = 0; j < used_matrix_size; j++)
			{
				mat_mul[i][j] = 0;
				for (k = 0; k < used_matrix_size; k++)
				{
					mat_mul[i][j] += matA[icalc + k] * matB_trans[j][k];
				}
			}
		}
		...
		val_check = used_matrix_size - 1;
		printf("mat_mul[%d][%d] = %8.3f\n",val_check,val_check, mat_mul[val_check][val_check]);
